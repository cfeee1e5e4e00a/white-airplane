...
from src.models.commutation_state import DEFAULT_HOUSE_COUNT, DEFAULT_RELAY_COUNT, DEFAULT_SUPPLY_COUNT
    ...
    for inout in ['IN', 'OUT']:
        for supply_id in range(DEFAULT_SUPPLY_COUNT):
            for house_id in range(DEFAULT_HOUSE_COUNT):
                for flat_id in range(DEFAULT_RELAY_COUNT // DEFAULT_HOUSE_COUNT 
                                     + (house_id >= DEFAULT_HOUSE_COUNT 
                                        - DEFAULT_RELAY_COUNT % DEFAULT_HOUSE_COUNT)):
                    for topic, schema in zip(SENSOR_TOPICS, 
                                            SENSOR_SCHEMAS):
                        msg = schema.format(
                                house_id=house_id,
                                flat_id=flat_id,
